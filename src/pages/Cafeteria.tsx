...
const Cafeteria = () => {
  ...
  const cafe = getCafeteriaById(id || '');
    ...
    <Layout>
      ...
      <div className="relative h-64 md:h-80 lg:h-96 bg-card">
        ...
        {cafe.images.length > 1 && (
          <>
            <button
              onClick={() => setCurrentImage((prev) => (prev === 0 ? cafe.images.length - 1 : prev - 1))}
              className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-background/80 backdrop-blur-sm flex items-center justify-center hover:bg-background transition-colors"
            >
              <ChevronLeft className="h-5 w-5" />
            </button>
            <button
              onClick={() => setCurrentImage((prev) => (prev === cafe.images.length - 1 ? 0 : prev + 1))}
          ...
          {cafe.images.map((_, i) => (
            <button
              ...
              className={cn(
                ...
                i === currentImage ? 'bg-primary w-4' : 'bg-foreground/50'
              )}
            />
          ))}
        </div>
      ...
      <div className="container mx-auto px-4 py-6 pb-32 md:pb-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
          <Card className="bg-card border-border">
            <CardContent className="p-4 text-center">
              ...
              <p className="text-2xl font-bold">{cafe.wifiSpeed}</p>
              <p className="text-xs text-muted-foreground">Mbps</p>
            </CardContent>
          </Card>
          <Card className="bg-card border-border">
            <CardContent className="p-4 text-center">
              <NoiseIcon className="h-6 w-6 text-primary mx-auto mb-2" />
              <p className="text-sm font-semibold">{noiseLabels[cafe.noiseLevel]}</p>
              <p className="text-xs text-muted-foreground">Ruido</p>
            </CardContent>
          </Card>
          <Card className="bg-card border-border">
            <CardContent className="p-4 text-center">
              <Zap className="h-6 w-6 text-status-warning mx-auto mb-2" />
              <p className="text-sm font-semibold">{cafe.hasOutlets ? 'Disponibles' : 'No hay'}</p>
              <p className="text-xs text-muted-foreground">Enchufes</p>
            </CardContent>
          </Card>
          <Card className="bg-card border-border">
            <CardContent className="p-4 text-center">
              <span className="text-2xl">{cafe.priceRange}</span>
              <p className="text-xs text-muted-foreground mt-1">Precios</p>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <Card className="bg-card border-border">
            <CardHeader>
              <CardTitle className="text-lg">Menú destacado</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-2">
        ...
        <Card className="bg-card border-border mt-6">
          <CardHeader>
            <CardTitle className="text-lg">Ocupación estimada</CardTitle>
          </CardHeader>
          <CardContent>
                ...
                <div key={label} className="flex flex-col items-center gap-2 flex-1">
                  <div className="w-full max-w-12 relative">
                    ...
                    className={cn(
                      ...
                      occupationValues[i] > 70 ? 'bg-destructive' :
                      occupationValues[i] > 40 ? 'bg-status-warning' : 'bg-status-success'
                    )}
                      style={{ height: `${occupationValues[i]}px` }}
                    />
                  </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="font-bold text-lg">{cafe.discount}% de descuento</span>
                  </div>
                  <p className="text-sm text-muted-foreground">{cafe.discountDescription}</p>
                </div>
                <Button asChild>
                  <Link to={`/mi-qr?cafe=${cafe.id}`}>
          ...
          <CardContent>
            <div className="aspect-video bg-secondary rounded-lg flex items-center justify-center">
              <div className="text-center text-muted-foreground">
                ...
                <p className="text-sm">{cafe.address}, {cafe.barrio}</p>
        ...
        <Card className="bg-card border-border mt-6">
          <CardHeader>
            <CardTitle className="text-lg">Reseñas de workers</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <div className="fixed bottom-20 left-0 right-0 p-4 bg-gradient-to-t from-background via-background to-transparent md:hidden">
          <Button variant="hero" size="lg" className="w-full" asChild>
            <Link to={`/mi-qr?cafe=${cafe.id}`}>
              ...
              Obtener {cafe.discount}% de descuento
            </Link>
          </Button>
        </div>
      )}
