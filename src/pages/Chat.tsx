...
const Chat = () => {
  ...
  const { data: allCafeterias = [] } = useCafeterias();
  ...
  const getAIResponse = async (userMessage: string): Promise<{ response: string; cafes: Cafeteria[] }> => {
    try {
      ...
      const matchedCafes = allCafeterias.filter(c => lowerResponse.includes(c.name.toLowerCase()));
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    (async () => {
      const { response, cafes } = await getAIResponse(currentInput);
          ...
          <div className="container mx-auto px-4 py-6 space-y-6">
            {messages.map((message) => (
              <div key={message.id} className={cn('flex gap-3 animate-fade-in', message.role === 'user' ? 'flex-row-reverse' : '')}>
                <div className={cn('w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0', message.role === 'assistant' ? 'bg-primary/20' : 'bg-secondary')}>
                  {message.role === 'assistant' ? <Bot className="h-4 w-4 text-primary" /> : <User className="h-4 w-4" />}
                </div>
                <div className={cn('max-w-[85%] md:max-w-[70%]', message.role === 'user' ? 'text-right' : '')}>
                  <div className={cn('rounded-2xl px-4 py-2.5', message.role === 'assistant' ? 'bg-card border border-border' : 'bg-primary text-primary-foreground')}>
            ...
            <div className="container mx-auto px-4 py-3">
              ...
              <div className="flex flex-wrap gap-2">
                {quickQuestions.map((q, i) => (
                  <button key={i} onClick={() => setInput(q)} className="text-xs px-3 py-1.5 rounded-full bg-secondary hover:bg-secondary/80 transition-colors">
                    {q}
                  </button>
                ))}
              </div>
        ...
        <div className="border-t border-border bg-card/50 backdrop-blur-sm">
          <div className="container mx-auto px-4 py-4">
            <form onSubmit={handleSubmit} className="flex gap-3">
              <input ref={inputRef} type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Ej: Necesito un lugar tranquilo en Palermo con buen WiFi..." className="flex-1 bg-secondary border border-border rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all" />
